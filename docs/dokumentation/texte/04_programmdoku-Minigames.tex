...
\begin{figure}[H]
	...
	\caption{Struktogramm der setAnswer-Methode (erstellt mit Structorizer)}
...
\begin{figure}[b]
	\centering
	\includegraphics[width=0.01\linewidth]{bilder/bro.jpg}
\end{figure} 
...
public boolean istGeloest() {
    // Glas muss vollständig gefuellt sein
    if (bloecke.size() != MAX_KAPAZITAET) return false;
    // Alle Blöcke muessen die gleiche Farbe haben
...
\begin{sloppypar}
    \begin{itemize}
        ...
        \item \texttt{draw(Graphics2D)}: Zeichnet Titel, Hinweistext, Timer und delegiert das Zeichnen der Gläser an die jeweiligen \texttt{Reagenzglas}-Objekte.
    \end{itemize}
\end{sloppypar}
...
private void umschuetten(int von, int nach) {
    ...
    // Alle aufeinanderfolgenden Blöcke gleicher Farbe umschütten
...
\begin{figure}[H]
    \centering
        \includegraphics[width=1\textwidth]{bilder/BeispielChemie.png}
        \caption{Sortierte und unsortierte Reagenzgläser}
        \label{fig:BeispielChemie}
\end{figure}
...
Das Spiel läuft in drei Phasen ab: \texttt{PHASE\_VORBEREITUNG}, in der der/die Spieler*in einen Kraftpfeil vom Ball wegzieht, \texttt{PHASE\_SIMULATION}, in der der Ball unter Einfluss der Beschleunigungen fliegt, und \texttt{PHASE\_ERGEBNIS}, die nach dem Treffem oder Verfehlen bzw der Kollison mit Boden oder Wand eintritt.
...
\begin{sloppypar}
    \begin{itemize}
        ...
        \item \texttt{draw(Graphics2D)}: Zeichnet Titel, Hinweistext bzw. Anleitung, Ziel und die Kraftpfeile.
    \end{itemize}
\end{sloppypar}
...
private void ballBewegen() {
    ...
    ballX += velX;
    ballY += velY;
    // Fensterrand/Wand getroffen -> verloren
...
\begin{figure}[H]
    \centering
        \includegraphics[width=1\textwidth]{bilder/BeispielPhysik.png}
        \caption{Eingestellter Pfeil der Spielerkraft}
        \label{fig:BeispielPhysik}
\end{figure}
...
De/die Spieler*in sieht vier zufällig ausgewählte Pflanzenbilder und muss diesen die richtigen Namen zuordnen, indem er/sie Labels anklickt und auf die passenden Bilder platziert.
...
\begin{sloppypar}
    \begin{itemize}
        ...
        \item \texttt{draw(Graphics2D g2)}: Zeichnet Titel und Hinweistext bzw. Anleitung.
    \end{itemize}
\end{sloppypar}
...
// Originalbild der gewählten Pflanze laden
BufferedImage original = (BufferedImage) chosen.get(i).image;

// Skalierungsfaktor berechnen: Bild soll den Slot vollständig ausfuellen
...
// Neues Bild in der skalierten Größe erstellen
BufferedImage scaled = new BufferedImage(scaledW, scaledH,
                                         BufferedImage.TYPE_INT_RGB);
Graphics2D g2d = scaled.createGraphics();

// Originalbild in die neue skalierte Größe zeichnen
...
// Bild mittig auf Slotgröße zuschneiden (Center-Crop)
...
// Zugeschnittenes Bild als BaseObjekt hinzufügen
...
// Objekt im Slot-Array speichern (für spätere Klick-Erkennung)
// und der Szene hinzufügen, damit es gezeichnet wird
...
// Ist kein Label ausgewählt, auf Klick eines Labels prüfen
if (selectedLabel == null) {
    for (BaseObject obj : objects) {
        if (obj instanceof BiologyLabelObject label) {
            if (label.isClicked()) {
                // Label als ausgewählt markieren und merken
                selectedLabel = label;
                label.selected = true;
            }
        }
    ...
    // Klick auf das bereits ausgewählte Label hebt Auswahl wieder auf
    ...
    for (int i = 0; i < 4; i++) {
        if (imagePlaceholders[i].isClicked()) {
            ...
            // Ausgewähltes Label auf den angeklickten Bildslot platzieren
            selectedLabel.x = imageX[i] * PIXEL_SCALE;
            selectedLabel.y = 71 * PIXEL_SCALE;
            selectedLabel.selected = false;
            selectedLabel = null;

            // Wenn alle vier Slots belegt sind, Gewinnprüfung starten
            if (allPlaced()) {
                if (checkWin()) {
                    levelWon();
                } else {
...
Eine Erweiterung von \texttt{BaseObject}, die ein anklickbares, beschriftetes Label darstellt, das der/die Spieler*in per Klick auswählen und auf einem Bildslot platzieren kann.
...
\begin{figure}[H]
    \centering
        \includegraphics[width=1\textwidth]{bilder/BeispielBiologie.png}
        \caption{Ausgewählte, unausgewählte und zugeordnete Label}
        \label{fig:BeispielBiologie}
\end{figure}